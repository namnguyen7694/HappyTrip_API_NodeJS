...
module.exports.login = (req, res, next) => {
  ...
  User.findOne({ email })
    .then(user => {
      if (!user)
        return Promise.reject({ status: 404, message: "Email không tồn tại" });
    ...
    .then(result => {
      ...
      const user = result[1];
      if (!isMatch)
        return Promise.reject({ status: 400, message: "Mật khẩu không chính xác" });
      const payload = {
        email: user.email,
      ...
      return jwtSign(payload, keys.secret_key, { expiresIn: 36000 });
    })
    .then(token => {
